...
fn sawtooth_cosine_fragments(period: f32, frequency: f32, order: u32) -> Option<Vec<WaveSoundSetting>> {
    if period <= 0f32 {
        return None;
    }
...
fn write_fromc4toc5() {
    ...
    let sound_settings = sawtooth_cosine_fragments(1f32, C5_FLOAT, 50).unwrap();
